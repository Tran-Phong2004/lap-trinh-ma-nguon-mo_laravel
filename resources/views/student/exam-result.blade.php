...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .score-circle.excellent .score-value { color: #28a745; }
        .score-circle.good .score-value { color: #17a2b8; }
        .score-circle.average .score-value { color: #ffc107; }
        .score-circle.poor .score-value { color: #dc3545; }
...
<body>
    <div class="container">
        ...
        <div class="header">
            <h1>📊 Kết Quả Thi</h1>
            <a href="{{ route('student.exam-sessions') }}" class="back-btn">← Về danh sách</a>
        </div>
        ...
        @if($latestResult)
            ...
            <div class="exam-info-card">
                ...
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $examSession->results()->count() }} / {{ $examSession->max_attempts }}</span>
                </div>
                <div class="info-row">
                        ...
                        <tbody>
                            @foreach($examSession->results()->orderByDesc('submitted_at')->get() as $index => $result)
                                <tr>
                                    ...
                                    <td>
                                        <strong style="color: {{ $result->score >= 80 ? '#28a745' : ($result->score >= 50 ? '#ffc107' : '#dc3545') }}">
                                            {{ round($result->score, 1) }} điểm
                                        </strong>
                                    </td>
                                    <td>
            ...
            @if($examSession->results()->count() < $examSession->max_attempts)
                <div style="margin-top: 20px; text-align: center;">
                    <form action="{{ route('student.start-exam', $examSession->id) }}" method="POST" style="display: inline-block;">
                        @csrf
                        <button type="submit" class="back-btn" style="background: #28a745; padding: 15px 40px; font-size: 16px;">
